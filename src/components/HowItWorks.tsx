          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            {steps.map((step, index) => (
              <div key={index} className="relative">
                <div className="bg-kayayoo-darkgray p-6 rounded-lg border border-kayayoo-darkgray hover:border-kayayoo-yellow/20 transition-all duration-300">
                  <div className="mb-4 flex justify-center">{step.icon}</div>
                  <h3 className="text-xl font-semibold mb-2 text-white text-center">{step.title}</h3>
                  <p className="text-gray-400 text-center">{step.description}</p>
                </div>
                <div className="hidden md:block absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-kayayoo-yellow text-black font-bold flex items-center justify-center">
                  {index + 1}
                </div>
              </div>
            ))}
